(function () {
  ...
  function addUI() {
    ...
    container.innerHTML = `
      ...
      <div class="clock-row" id="cp-clock-row" style="width:100%;display:flex;justify-content:center;align-items:center;background:#f8fafc;border-bottom:1px solid #e0e7ef;">
  ...
  //      contentDiv.style.display = 'none';
    ...
    let stopwatchInterval, stopwatchSeconds = 0, running = false;
    ...
    resetBtn.onclick = () => {
      if (mode === 'timer') {
        clearInterval(timerInterval);
        timerRemaining = (parseInt(minInput.value, 10) || 0) * 60 + (parseInt(secInput.value, 10) || 0);
        updateClockDisplay(timerRemaining);
        setState('idle');
        running = false;
        timerPaused = false;
    ...
    // Resume logic for timer/stopwatch
    startBtn.addEventListener('click', () => {
      if (mode === 'timer' && timerPaused && !running) {
        running = true;
        setState('running');
        timerInterval = setInterval(() => {
          timerRemaining--;
        ...
        timerPaused = false;
      } else if (mode === 'stopwatch' && stopwatchPaused && !running) {
        running = true;
        setState('running');
        stopwatchInterval = setInterval(() => {
          stopwatchSeconds++;
    ...
    function renderBookmarks() {
      chrome.storage.sync.get(["cp_bookmarks"], (res) => {
        ...
        if (bookmarks.length === 0) {
          bookmarksSection.innerHTML = '<div style="color:#888;font-size:0.97em;">No bookmarks yet.</div>';
          return;
        }
        bookmarksSection.innerHTML = bookmarks.slice().reverse().map(b => `
          <div style="background:#fff;border-radius:6px;box-shadow:0 1px 4px #0001;margin-bottom:8px;padding:7px 10px;">
            <a href="${b.url}" target="_blank" style="font-weight:bold;color:#1a2233;text-decoration:none;">${b.title}</a>
            <span style="font-size:0.92em;color:#fff;background:#f5c242;border-radius:4px;padding:1px 6px;margin-left:6px;">${b.status}</span><br/>
            <span style="color:#222;font-size:0.97em;">${b.note ? b.note : '<i>No note</i>'}</span><br/>
            <span style="color:#888;font-size:0.85em;">${new Date(b.date).toLocaleString()}</span>
          </div>
        `).join('');
      });
    }
    // Show/hide content and bookmarks, but clock always visible
    toggleBtn.onclick = () => {
      container.classList.toggle('collapsed');
      toggleBtn.innerHTML = container.classList.contains('collapsed') ? '&#9660;' : '&#9650;';
      // Clock row always visible
